...
private:
	...
	int m_nCurFrame;		// last (current) frame played
	...
	std::string m_strAnimName;	// amimation (propert) name
...
#define _DECODE_SHEET s.m_from.m_rowcol.m_grid.m_numCols, s.m_from.m_rowcol.m_grid.m_numRows, s.m_from.m_rowcol.m_bHorizontally ? s.m_from.iFrom : s.m_from.m_rowcol.m_iRowCol, s.m_from.m_rowcol.m_bHorizontally ? s.m_from.m_rowcol.m_iRowCol : s.m_from.iFrom
	...
	bool IsAnimationPlaying();							// returns true is any animation is currently played by the sprite
	bool IsAnimationPlaying(char* pPropName);			// returns true is the animation provided as pPropName is currently played by the sprite
	const char* GetCurrentAnimation();					// returns the string name of the animation currently played by the sprite, or NULL if none
	int GetCurrentAnimationFrame();						// returns current frame index of the currently played animation, or -1 is none is playing
